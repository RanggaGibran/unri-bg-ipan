...
export async function getStudentById(id: number) {
  try {
    ...
    if (error) {
      ...
      const student = mockStudents.find(s => s.id === id)
      return { data: student || null, error }
    }
...
export async function getActiveStudents() {
  try {
    if (!isSupabaseConfigured()) {
      const activeStudents = mockStudents.filter(s => !s.is_completed)
    ...
    if (error) {
      ...
      const activeStudents = mockStudents.filter(s => !s.is_completed)
      return { data: activeStudents, error }
    }
...
export async function getCompletedStudents() {
  try {
    if (!isSupabaseConfigured()) {
      const completedStudents = mockStudents.filter(s => s.is_completed)
      return { data: completedStudents, error: null }
    }
    ...
    if (error) {
      ...
      const completedStudents = mockStudents.filter(s => s.is_completed)
      return { data: completedStudents, error }
    }
...
export async function getStatistics() {
  try {
    if (!isSupabaseConfigured()) {
      return { data: mockStatistics, error: null }
    }
    ...
    if (allError) {
      ...
      return { data: mockStatistics, error: allError }
    }
    ...
    return { data: mockStatistics, error }
  }
}
